...
`;
    const pressure_frag = `
		...
		varying vec2 uv;
		void main(){
		float p0 = texture2D(pressure, uv + vec2(px.x * 1.0, 0.0)).r;
		float p1 = texture2D(pressure, uv - vec2(px.x * 1.0, 0.0)).r;
		float p2 = texture2D(pressure, uv + vec2(0.0, px.y * 1.0)).r;
		float p3 = texture2D(pressure, uv - vec2(0.0, px.y * 1.0)).r;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        const centerX = Math.min(
          Math.max(Mouse.coords.x, -1 + cursorSize * cellScale.x + cellScale.x * 2),
          1 - cursorSize * cellScale.x - cellScale.x * 2
        );
        const centerY = Math.min(
          Math.max(Mouse.coords.y, -1 + cursorSize * cellScale.y + cellScale.y * 2),
          1 - cursorSize * cellScale.y - cellScale.y * 2
        );
