...
export const NexusPanel: React.FC<NexusPanelProps> = ({ onBack, onLogout, catalog, setCatalog, evaluations }) => {
  ...
  const downloadNormsTemplate = () => {
    const template = [
      { test: 'MFC', pais: 'AR', año: 2025, sexo: 'M', p10: 45, p25: 60, p50: 75, p75: 90, p90: 110, p95: 120, detalle: 'Media normal central' },
      { test: 'MFC', pais: 'AR', año: 2025, sexo: 'F', p10: 48, p25: 62, p50: 78, p75: 92, p90: 112, p95: 122, detalle: 'Desviación leve superior' },
      { test: 'BART', pais: 'CL', año: 2024, sexo: 'M', p10: 30, p25: 45, p50: 60, p75: 75, p90: 95, p95: 105, detalle: 'Tolerancia al riesgo base' },
    ];
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-black uppercase tracking-[0.3em] flex items-center gap-3">
              <Database className="w-6 h-6 md:w-8 md:h-8 text-[#7B2CBF]" /> CONFIGURACIÓN CENTRAL
            </h1>
            <div className="flex items-center gap-3 mt-1 text-[9px] text-white/30 uppercase tracking-[0.4em]">
              <span className="text-[#00F3FF] font-black">ADMINISTRADOR DEL SISTEMA</span>
              <span className="hidden md:inline">•</span>
              <span className="hidden md:inline">Versión 2.5</span>
            </div>
          </div>
        </div>
      </header>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {catalog.map(test => (
                    <div key={test.id} className="bg-white/[0.02] border border-white/10 p-8 rounded-[32px] relative group hover:border-white/20 transition-all">
                      ...
                      <div className="text-[9px] font-black uppercase tracking-widest mb-4" style={{ color: test.color }}>{test.type.toUpperCase()}</div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-[10px] uppercase tracking-widest">
                      ...
                      <tbody className="divide-y divide-white/5">
                        {norms.filter(n => {
                          const matchesTest = normsFilters.testId === 'all' || n.test === catalog.find(t => t.id === normsFilters.testId)?.title || n.test === normsFilters.testId;
                          const matchesCountry = normsFilters.country === 'all' || n.pais.toLowerCase() === normsFilters.country.toLowerCase();
                          // Simulación de filtro por año (en el mock solo hay 2024 y 2025)
                          const matchesYear = normsFilters.year === 'all' || (normsFilters.year === 'last5' && n.año >= 2020) || n.año.toString() === normsFilters.year;
            ...
            {activeTab === 'abm' && (
              <motion.div key="abm" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-white/[0.03] border border-white/10 rounded-[40px] overflow-hidden">
                <div className="p-8 border-b border-white/10 flex justify-between items-center">
                  <h2 className="text-xl font-black uppercase tracking-widest">Gestión de Usuarios</h2>
                  <div className="flex gap-4">
                    <div className="relative">
                      <Search className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-white/20" />
                      <input className="bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-2 text-[10px] uppercase font-bold focus:border-[#00F3FF] outline-none w-64" placeholder="Buscar ID / Usuario..." />
                    </div>
                  </div>
                </div>
                <table className="w-full text-left text-[11px] uppercase tracking-widest">
                  <thead className="bg-white/5 text-white/30 font-bold">
                    <tr>
                      <th className="p-6">Usuario / Alias</th>
                  ...
                  <tbody className="divide-y divide-white/5">
                    ...
                    <UserRow id="ID_9901" alias="HR_Lead_Global" rol="Corporativo" level="C2" status="Pending" />
                  </tbody>
                </table>
              </motion.div>
            )}
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-[#080A0F]/90 backdrop-blur-md overflow-y-auto">
            ...
            >
              ...
              <div className="space-y-10">
                ...
                >
                  <Save className="w-5 h-5" /> Guardar Protocolo
                </button>
              </div>
            </motion.div>
          </div>
            ...
            >
              ...
              <form onSubmit={handleUploadNorms} className="space-y-8">
                ...
                <div className="pt-4">
                  ...
                  >
                    <Save className="w-5 h-5" /> Confirmar Ingesta de Datos
                  </button>
                </div>
              </form>
            </motion.div>
